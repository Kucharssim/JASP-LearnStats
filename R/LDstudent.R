...
LDstudent <- function(jaspResults, dataset, options, state=NULL){
  ...
  options <- .recodeOptionsLDStudent(options)
...
.recodeOptionsLDStudent <- function(options){
  options[['pars']] <- list(ncp = options[['ncp']], df = options[['df']])
